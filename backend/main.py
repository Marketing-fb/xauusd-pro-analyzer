...
def fetch_live_price(symbol: str) -> float:
    ...
    price = SYMBOL_MAP.get(symbol, {}).get("base_price", 2748.50)
    if symbol == "XAUUSD":
        ctx = ssl.create_default_context()
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
        
        # 1. Try Binance PAXG (Gold 1:1, 100% uptime, zero rate limit)
        try:
            req0 = urllib.request.Request("https://api.binance.com/api/v3/ticker/price?symbol=PAXGUSDT", headers={"User-Agent": "Mozilla/5.0"})
            with urllib.request.urlopen(req0, context=ctx, timeout=3) as resp0:
                data0 = json.loads(resp0.read().decode())
                if "price" in data0:
                    price = round(float(data0["price"]), 2)
                    LIVE_PRICE_CACHE[symbol] = (price, now)
                    return price
        except Exception as e:
            print(f"binance paxg fetch fallback: {e}")

        # 2. Try Gold API fallback
        try:
